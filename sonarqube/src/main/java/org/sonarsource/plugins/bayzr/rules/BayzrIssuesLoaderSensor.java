...
public class BayzrIssuesLoaderSensor implements Sensor {
	...
	protected static final String REPORT_LASTID_KEY = "sonar.bayzr.lastid";
	...
	protected String reportLatId() {
		return REPORT_USER_KEY;
	}

	protected String getReportLastId() {
		String reportLastId = settings.getString(reportLatId());
		if (!StringUtils.isEmpty(reportLastId)) {
			return reportLastId;
		} else {
			return "no";
		}
	...
	public void execute(final SensorContext context) {
		if (!StringUtils.isEmpty(getReportUrl()) && !StringUtils.isEmpty(getReportUser())
				&& !StringUtils.isEmpty(getReportPass())) {
	...
	protected void parseAndSaveResults(String url, String user, String pass, String last_id) throws XMLStreamException {
		LOGGER.info("Parsing 'BayZR' Analysis Results");
		BayzrAnalysisResultsParser parser = new BayzrAnalysisResultsParser();
		List<BayzrError> errors = parser.parse(url, user, pass, last_id);
	...
	private void saveIssue(final InputFile inputFile, int line, final String externalRuleKey, final String message) {
		RuleKey ruleKey = RuleKey.of(BayzrRulesDefinition.getRepositoryKeyForLanguage(inputFile.language()),
				externalRuleKey);
	...
	public List<BayzrError> parse(String url, String user, String pass, String last_id) throws XMLStreamException {
      ...
      try{
        ...
        ResultSet rs;
        if (last_id=="yes"){
           rs = stmt.executeQuery("select last_build_id from bayzr_last_check where checker='sonarqube'");
           int last_build_id = 0;
           ...
           rs = stmt.executeQuery("select bayzr_err, severity, file, pos, descript from bayzr_err_list where build_number = (select max(id) from bayzr_build_info where completed = 1 and id >= " + str_build + ") order by file, pos");
        	...
        	if (build_nm != null) {
                ...
                rs = stmt.executeQuery("select bayzr_err, severity, file, pos, descript from bayzr_err_list where build_number = " + str_build + " order by file, pos");
        	} else {
        		rs = stmt.executeQuery("select bayzr_err, severity, file, pos, descript from bayzr_err_list where build_number = 0 order by file, pos");
        	}
        }
        while(rs.next()){
